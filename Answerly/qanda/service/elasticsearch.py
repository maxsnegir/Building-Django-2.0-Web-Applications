...
def get_client():
    return Elasticsearch(hosts=[{'host': settings.ES_HOST,
...
def bulk_load(questions):
    ...
    for ok, result in streaming_bulk(
            get_client(), es_questions, index=settings.ES_INDEX,
...
def upsert(question_model):
    ...
    response = client.update(settings.ES_INDEX, question_model.id,
                             doc_type=doc_type,
                             body={
                                 'doc': question_dict,
                                 'doc_as_upsert': True,
